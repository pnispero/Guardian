%%% THIS is the original script, but I added some minimal comments %%%%
%% see your email from tonee for more info %%
...
% measurement, for use by the MPS to protect the SXRSS grating.
...
disp('FELpulseEnergyMonitor.m, v2.22 02/10/2022');
% 02/10/22 Add UNDS Launch setpoints, temp disable BC2 Pk Curr state trip
...
while 1 % Loop forever
    ...
    for iund = 1:9
        ...
        % PATRICK Comment:
        % undulator_K_n() is not a function, its a field of type array
        % but the () is used to index so it. Likewise 'und_Kvalue' 
        % is not a function either, it is an array.
    end
    ...
    if stats.saveNewSnapshot || cycle == 1 % we want to force-save a snapshot of FEL parameters on startup
        ...
        for iulsp = 1:4
            idx = 66 + iulsp;
            output.pv{idx,1} = L.pv{L.UNDlaunch_setpt_store_n(iulsp), 1};
            output.value(idx,1) = stats.UNDlaunch_setpt(iulsp);
        end
        % and then set the "save FEL pulse energy params" bit back to 0
        output.pv{71,1} = L.pv{L.saveNewSnapshot_n, 1};
        output.value(71,1) = 0;
        % and set the Protect the SXRSS Grating PV to 1 (protect)
        output.pv{72,1} = L.pv{L.protectSXRSSgrating_n, 1};
        output.value(72,1) = 1;
        %
        try
...
for iulsp = 1:4
    stored_UNDlaunchSP(iulsp) = d(L.UNDlaunch_setpt_store_n(iulsp));
end
stored.UNDlaunch_setpt   = stored_UNDlaunchSP;
%
...
for iulsp = 1:4
    ...
    L.UNDlaunch_setpt_store_n(iulsp) = n;
end
%
...
L.LH2_delay_n = n;
n = n + 1;
L.pv{n,1} = 'LASR:IN20:475:PWR1H'; % Laser heater power, Archiver 1Hz
L.LH_power_n = n;
% QUADS dammit
...
for iulsp = 1:4
    n = n + 1;
    UNDLspname = strcat('FBCK:FB04:TR02:S', num2str(iulsp), 'DES');
    L.pv{n,1} = UNDLspname;
    L.UNDlaunch_setpt_n(iulsp) = n;
end
n = n + 1;
...
% Check if BC2 current feedback is on, then verify that the BLEN reading isn't bonkers.
% If it's ok, check if SXR dualE is on. Using the appropriate setpoint,
...
if qq > (tols + QQ) || qq < (QQ - tols)
    trip = 1;
    out.message = 'DL2 vernier beyond allowed range. Check FEL pulse energy.';
    return
end
...
for iulsp = 1:4
    ...
    if abs(QQ - qq) > abs(tols*QQ)
        trip = 1;
        out.message = 'No tweaking UND launch setpoints!';
    end
end
end
%end
